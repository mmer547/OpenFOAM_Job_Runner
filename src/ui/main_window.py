...
class MainWindow(QMainWindow):
    ...
    def _setup_menu(self) -> None:
        ...
        tools_menu.addAction("ケースレポート...", self._open_case_report)
    ...
    def _setup_toolbar(self) -> None:
        ...
        report_btn.setText("📄 ケースレポート")
        report_btn.clicked.connect(self._open_case_report)
    ...
    def _open_case_report(self) -> None:
        job_id = self._selected_job_id()
        if job_id is None:
            QMessageBox.information(self, "情報", "ジョブを選択してください。")
            return
        job = next((j for j in self._scheduler.jobs if j.id == job_id), None)
        if job is None:
            return
        dialog = ReportDialog(job, self)
        dialog.exec()
